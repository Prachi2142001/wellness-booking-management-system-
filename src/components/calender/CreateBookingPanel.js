...
const CreateBookingPanel = ({ createData, onClose }) => {
  const [form, setForm] = useState({
    ...
    duration: 60,
    start: createData?.start || "09:30 AM",
  });
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center px-5 py-3 border-b border-gray-100 bg-[#fafafa]">
        <h2 className="text-[14px] font-semibold text-[#111827]">New Booking</h2>
        <button
          onClick={onClose}
      ...
      <div className="flex-1 text-[13px] bg-[#fafafa] overflow-y-auto w-full">
        ...
        <div className="grid grid-cols-2 border-b border-gray-100 bg-white">
          ...
          <div className="px-5 py-4 flex items-center">
            ...
            <span className="font-semibold text-[#111827]">09:30 PM</span>
          </div>
        </div>

        {!selectedClient && (
          <div className="flex-1 bg-white">
            <div ref={ref} className="relative z-50 bg-white px-5 py-4 border-b border-gray-100">
              <input
                    ...
                    filteredClients.map((c, i) => (
                      <div
                        ...
                        className={`px-5 py-3.5 cursor-pointer border-b border-gray-50 last:border-none transition-colors ${
                          i === 0 ? "bg-[#3C2212] text-white" : "hover:bg-gray-50 bg-white text-[#111827]"
                        }`}
                      >
        ...
        {selectedClient && (
          <>
              ...
              <div className="flex gap-4 items-start w-full">
                <div className="relative mt-1 shrink-0">
                   <div className="w-[42px] h-[42px] rounded-full bg-[#E59850] flex items-center justify-center text-white font-medium text-[16px]">
                     ...
                     {selectedClient.name?.split(" ")[1]?.[0]?.toUpperCase() || "B"}
                   </div>
                ...
                <div className="flex-1 w-full flex flex-col">
                  <div className="flex justify-between items-start w-full">
                    <p className="font-semibold text-[#111827] text-[14px] leading-tight flex-1">
                      {selectedClient.phone} <span className="text-gray-500 font-medium">({selectedClient.id && `#${selectedClient.id}`})</span><br/>
                      <span className="inline-block mt-0.5">{selectedClient.name}</span>
                    </p>
                    <span 
            ...
            <div className="px-5 py-5 border-b border-gray-100 bg-white space-y-4">
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-2 flex-1">
                  ...
                  {/* Figma screenshot does show a tiny subtle chevron next to the 60 Mins text but it could just be dropdown, we leave it */}
                </div>
              </div>
              ...
              <div className="flex text-[13px] items-center text-gray-400 italic gap-8 pt-1">
                <div className="flex items-center gap-2">
                  <span>For:</span>
                  <span className="text-[#111827] font-semibold not-italic">{form.duration} min</span>
                </div>
                
                <div className="flex items-center gap-2">
                  <span>At:</span>
                  <span className="text-[#111827] font-semibold not-italic flex items-center gap-1.5">
                    {form.start} <DropdownIcon className="w-[9px] h-[9px] text-gray-500 ml-0.5" />
                  </span>
                </div>
              </div>
            ...
            {/* Added extra padding bottom to space things nicely like in Figma */}
